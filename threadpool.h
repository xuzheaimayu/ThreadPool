...
{
	MODE_FIXED,//固定数量的线程
	MODE_CACHED,//可动态增长的的线程池
};

class Thread
{
public:
	//使用std::function来定义线程函数类型
	using ThreadFunc = std::function<void(int)>; //线程函数类型，参数是线程id
...
private:
	ThreadFunc func_;
	static int generateId_;//用于生成线程ID
	int threadId_;//线程ID
};
...
public:
	...
	//向线程池中提交任务,使用可变参模板编程，可以接受任意函数和任意数量的参数
